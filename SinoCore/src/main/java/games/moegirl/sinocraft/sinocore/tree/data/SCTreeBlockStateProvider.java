...
public class SCTreeBlockStateProvider extends AbstractBlockStateProvider {
    ...
    public SCTreeBlockStateProvider(PackOutput output, String modid, ExistingFileHelper exHelper, List<TreeType> treeTypes) {
    ...
    protected void registerStatesAndModels() {
        for (var tree : treeTypes) {
            ...
            simpleBlock(tree.getBlock(TreeBlockType.POTTED_SAPLING),
                    models().singleTexture(TreeBlockType.POTTED_SAPLING.makeRegistryName(tree.getName()).getPath(),
                            blockLoc(mcLoc("/flower_pot_cross")),
                            blockLoc(TreeBlockType.SAPLING.makeRegistryName(tree.getName()))));

            ResourceLocation planksTextures = blockTexture(tree.getBlock(TreeBlockType.PLANKS));

            ModelFile sign = models().sign(TreeBlockType.SIGN.makeRegistryName(tree.getName()).getPath(), planksTextures);
            ...
            ResourceLocation doorName = TreeBlockType.DOOR.makeRegistryName(tree.getName());
